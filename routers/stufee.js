...
StufeeRouter.get('/', verifyToken, (req, res) => {
    ...
    if (req.params.id) {
        let strQuery = `select std.father_name, std.phone,std.email ,cl.name,sc.name, stf.* from stufee_log stf 
INNER JOIN student_log std on std.uid=stf.uid 
INNER JOIN class_log cl on std.class_id = cl.id
INNER JOIN section_log sc on std.section_id = sc.id
where stf.uid=${req.params.uid} and stf.school_id=${req.params.id};`
